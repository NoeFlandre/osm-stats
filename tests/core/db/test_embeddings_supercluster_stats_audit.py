"""Audit tests for the supercluster element-type stats on the
embeddings pipeline.

Same shape as the TF-IDF audit suite
(tests/core/db/test_supercluster_stats_audit.py), but pointed at
the embeddings artifacts:
  ...
  output/standardize_first/embeddings/cluster_memberships_embeddings.csv

The TF-IDF pipeline produced 8,832 real clusters + 78,270 noise from
225,684 standardized tags. The embeddings pipeline (potion-base-8M)
produced 4,954 real clusters + 106,498 noise from 225,685
standardized tags.

The embeddings XLSX has 433 base keys (slightly different from the
TF-IDF 427). The user labeled 169 as 'yes' in this file. The audit
tests verify that the function's output for those 169 yes-labeled
base keys is 100 % correct.
"""
...
EMBEDDINGS_DIR = Path("output/standardize_first/embeddings")
MEMBERSHIPS = EMBEDDINGS_DIR / "cluster_memberships_embeddings.csv"
MEDOIDS = EMBEDDINGS_DIR / "cluster_medoids_embeddings.csv"
XLSX = EMBEDDINGS_DIR / "base_key_families.xlsx"
...
pytestmark = pytest.mark.skipif(
    not _has_real_data(),
    reason="real source DB / cache / embeddings outputs not available",
)
...
def source_db_aggregate() -> pd.DataFrame:
    ...
    per-element-type columns. Uses the cache as a JOIN filter to
    avoid the 300s+ full scan. The TRIM matches the cache's
    STANDARDIZED_KEY_EXPR / STANDARDIZED_VALUE_EXPR (see
    src/core/storage/cache.py).
    """
...
@pytest.fixture(scope="module")
def supercluster_df(yes_labels) -> pd.DataFrame:
    """Run the function once. ~180s on the full 192M-row source DB."""
...
def test_xlsx_has_exactly_169_yes_labels(yes_labels) -> None:
    """The user labeled 169 base keys as 'yes' in the embeddings
    XLSX. Lock the count down."""
    assert len(yes_labels) == 169, f"expected 169 yes labels, got {len(yes_labels)}"
...
def test_headline_numbers(supercluster_df) -> None:
    ...
    169 yes-labeled base keys (embeddings pipeline):
      - 118 polygon-friendly, 51 point-heavy
      - 798,138,011 polygon-friendly occurrences
      - 180,476,035 point-heavy occurrences
      -  15,683 polygon-friendly tags
      -   1,929 point-heavy tags
      -     359 polygon-friendly real clusters
      -     152 point-heavy real clusters
    """
    n_pf = int(supercluster_df["is_polygon_friendly"].sum())
    n_ph = int((~supercluster_df["is_polygon_friendly"]).sum())
    assert n_pf == 118, f"polygon-friendly: expected 118, got {n_pf}"
    assert n_ph == 51, f"point-heavy: expected 51, got {n_ph}"
    ...
    assert occ_pf == 798_138_011, f"occ PF: expected 798,138,011, got {occ_pf:,}"
    assert occ_ph == 180_476_035, f"occ PH: expected 180,476,035, got {occ_ph:,}"
    ...
    assert tags_pf == 15_683, f"tags PF: expected 15,683, got {tags_pf:,}"
    assert tags_ph == 1_929, f"tags PH: expected 1,929, got {tags_ph:,}"
    ...
    assert clusters_pf == 359, f"clusters PF: expected 359, got {clusters_pf:,}"
    assert clusters_ph == 152, f"clusters PH: expected 152, got {clusters_ph:,}"


def test_percentages_sum_to_100(supercluster_df) -> None:
    """pct_nodes + pct_ways + pct_relations ≈ 100 for every row.

    Same drift allowance as the TF-IDF suite (see
    tests/core/db/test_supercluster_stats_audit.py).
    """
    total_pct = (
...
def test_total_clusters_across_superclusters_matches_unique_real_clusters(
    ...
    """Sum of n_clusters across all 169 superclusters = number of
...
def test_embeddings_pipeline_produces_correct_global_totals() -> None:
    """The embeddings cluster_memberships file has 4,954 real
    clusters + 106,498 noise = 225,684 total rows. The blog post
    reports the real-clusters count (4,954) and noise (106,498);
    the total is 225,684 (same as the TF-IDF pipeline -- both
    pipelines run on the same standardize-first cache)."""
    mem = pd.read_csv(MEMBERSHIPS)
    n_total = len(mem)
    n_noise = int((mem["cluster_id"] == -1).sum())
    n_real = n_total - n_noise
    assert n_total == 225_684, f"total memberships: expected 225,684, got {n_total:,}"
    assert n_noise == 106_498, f"noise rows: expected 106,498, got {n_noise:,}"
    # Distinct real cluster_ids
    assert mem.loc[mem["cluster_id"] != -1, "cluster_id"].nunique() == 4_954, (
        f"distinct real cluster_ids: expected 4,954"
    )
